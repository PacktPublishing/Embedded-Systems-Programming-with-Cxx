...
Files related to **Example12** (Reading from SQLite database using statements)
...
* `step06_inspect_the_output.sh`: Script to inspect the output (`example_output.json`)
...
6. Run `source step06_inspect_the_output.sh` to print the generated JSON file (`example_output.json`)
...
Generate a different SQLite database with the `Example09` and replace the `example_input.db`, then run the binary again.
